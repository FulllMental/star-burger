...
Когда данные о ресторанах будут заполнены через [Панель администратора](http://127.0.0.1:8000/admin)
необходимо заполнить базу данных, отвечающую за координаты мест командой:
```sh
...
- `GEOPY_API_KEY` — который необходимо получить в [Кабинете разработчика](https://developer.tech.yandex.ru/services) для использования API определяющую рассотяние по координатам
